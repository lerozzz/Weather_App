...
function App() {
  ...
  const seacrhPressed = async (input) => {
    setCityName(input)
    const [cityInfo] = await fetch(`${api.geocoding}?q=${input}&appid=${api.key}`)
      .then((res) => res.json())

    const { lat, lon } = cityInfo


    const weatherResponse = await fetch(`${api.weather}?lat=${lat}&lon=${lon}&appid=${api.key}&units=metric`)
      .then((res) => res.json())
